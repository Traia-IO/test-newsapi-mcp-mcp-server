...
# Script to build and run the test-newsapi-mcp MCP Server locally in Docker
...
IMAGE_NAME="test-newsapi-mcp-mcp-server"
CONTAINER_NAME="test-newsapi-mcp-mcp-local"
HOST_PORT=${PORT:-8000}
CONTAINER_PORT=${PORT:-8000}

echo -e "${BLUE}🚀 Building and running test-newsapi-mcp MCP Server...${NC}"
echo
...
if [ ! -f .env ]; then
    ...
    if [ -f .env.example ]; then
        ...
        cat > .env << EOF
# test-newsapi-mcp MCP Server Configuration
PORT=8000
STAGE=MAINNET
LOG_LEVEL=INFO

# API Authentication (server's internal key for payment mode)
TEST_NEWSAPI_MCP_API_KEY=
...
if ! grep -q "^TEST_NEWSAPI_MCP_API_KEY=[^[:space:]]" .env; then
    if [ -t 0 ]; then
        echo ""
        echo -e "${YELLOW}📝 Please enter your test-newsapi-mcp API key:${NC}"
        echo -e "${YELLOW}   (This is the server's internal API key used when clients pay via 402)${NC}"
        read -p "TEST_NEWSAPI_MCP_API_KEY: " API_KEY_VALUE
        if [ -n "$API_KEY_VALUE" ]; then
            if grep -q "^TEST_NEWSAPI_MCP_API_KEY=" .env; then
                if [[ "$OSTYPE" == "darwin"* ]]; then
                    sed -i '' "s|^TEST_NEWSAPI_MCP_API_KEY=.*|TEST_NEWSAPI_MCP_API_KEY=$API_KEY_VALUE|" .env
                else
                    sed -i "s|^TEST_NEWSAPI_MCP_API_KEY=.*|TEST_NEWSAPI_MCP_API_KEY=$API_KEY_VALUE|" .env
                fi
            else
                echo "TEST_NEWSAPI_MCP_API_KEY=$API_KEY_VALUE" >> .env
            fi
        ...
        echo -e "${YELLOW}⚠️  Non-interactive mode: Please set TEST_NEWSAPI_MCP_API_KEY in .env manually${NC}"
    fi
else
    echo -e "${GREEN}✅ TEST_NEWSAPI_MCP_API_KEY already set in .env${NC}"
fi
...
echo
echo -e "${GREEN}✅ test-newsapi-mcp MCP Server is running!${NC}"
echo
